...
## 응답 형식 (매우 중요)
- 순수 텍스트로만 응답하세요. 절대 JSON, 코드블록(\`\`\`), 중괄호({}), "reply" 같은 필드명을 노출하지 마세요.
- 답변은 사용자에게 그대로 보여지는 자연스러운 한국어 문장이어야 합니다.
- 상담사 연결을 추천하고 싶을 때에만 답변의 맨 마지막 줄에 정확히 다음 태그 한 줄을 추가하세요: [[RECOMMEND_COUNSELOR]]
- 그 외에는 이 태그를 절대 붙이지 마세요. 태그를 붙이는 경우에도 태그 앞의 본문은 자연스러운 답변이어야 합니다.`;
...
export const sendMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const locationLine = sessionRow
      ? `\n\n## 사용자 지역\n- 사용자의 대략적 지역: ${sessionRow.sido} ${sessionRow.gugun}\n- 상담사 연결을 추천하는 응답(즉 [[RECOMMEND_COUNSELOR]] 태그를 붙이는 응답)에서는 답변 본문 안에 이 지역 기준으로 도움 받을 수 있는 공공 상담 자원 유형(예: "○○ 지역 정신건강복지센터", "청년마음건강센터", "대학교 학생상담센터") 2~3곳을 짧게 자연스럽게 소개하세요. 실제 존재가 확실하지 않은 특정 병원/기관 이름은 만들어 내지 마세요. 마지막 문장에서는 "아래 창에서 근처 상담센터를 바로 확인할 수 있어" 같이 하단 팝업을 부드럽게 언급하세요.`
      : "";
